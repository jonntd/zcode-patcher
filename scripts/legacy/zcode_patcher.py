...
四、模型拉取补丁（app.asar，--modelhub）
  ...
  （modelhub_payload.json），逻辑为本脚本重新实现（外科手术式按条目还原，
  不整包覆盖备份）。给设置→模型供应商的添加/编辑渠道页注入「拉取模型」按钮、
  请求头模拟面板、视觉探测，以及删除墓簿 zcode.deletedModels 持久化、
  渠道 headers 粘性。改动 preload/main/renderer styles 三个条目。
  原始字节记录在 app.asar.modelhub-patch.json（sidecar），可定点还原。
...
用法：
  ...
  python zcode_patcher.py --modelhub            # 模型拉取补丁（同样支持 --check/--revert）
  python zcode_patcher.py --edit-all            # 全消息可编辑（同样支持 --check/--revert）
...
def _repack_asar(asar: Path, overwrite: dict[str, bytes], remove: set[str],
                 tmp_suffix: str = ".tps-tmp") -> int:
    ...
    tmp = asar.with_name(asar.name + tmp_suffix)
...
def _refresh_sidecars_after_repack(asar: Path) -> None:
    """重打包后数据区位移/尺寸变化：
    ...
    if side.is_file():
        ...
        for r in recs:
            ...
            if hit and hit[1] == r.get("size") and (r.get("offset") != hit[0] or r.get("asar_size") != cur):
                r["offset"], r["asar_size"] = hit[0], cur
                changed = True
        if changed:
            side.write_text(json.dumps({"patches": recs}, ensure_ascii=False), encoding="utf-8")
            print(f"[*] 已同步 {side.name} 的 offset/指纹到重打包后的 asar")

    # modelhub sidecar：files 按 path 定位即可，只刷 asar_size
    side = asar.with_name(asar.name + ".modelhub-patch.json")
    if side.is_file():
        try:
            rec = json.loads(side.read_text(encoding="utf-8"))
        except Exception:
            rec = None
        files = (rec or {}).get("files") or []
        if rec and rec.get("asar_size") != cur and files and all(entries.get(f.get("path")) for f in files):
            rec["asar_size"] = cur
            side.write_text(json.dumps(rec, ensure_ascii=False), encoding="utf-8")

    # TPS sidecar：无 offset，只刷 asar_size（index 条目仍在即记录可信）
    ...
    if side.is_file():
        try:
            ...
            rec = None
        if rec and rec.get("asar_size") != cur and entries.get(rec.get("index_path")):
            rec["asar_size"] = cur
            side.write_text(json.dumps(rec, ensure_ascii=False), encoding="utf-8")
...
def process_tps_footer(asar: Path, check_only: bool, revert: bool, tps_src: Path | None) -> None:
    ...
    if revert:
        ...
        _refresh_sidecars_after_repack(asar)
...
# ------------------------------------------------- 模型拉取补丁（--modelhub，asar 重打包级）
# 注入载荷来自 https://github.com/CSSZYF/zcode-modelhub-patch (v1.2.1, MIT)：
# modelhub_payload.json 与本脚本同目录分发。三个补丁点：preload 暴露 IPC、
# main 追加处理器、renderer 注入「拉取模型/请求头模拟/视觉探测」UI 与删除持久化。
# 与上游实现的两点差异：renderer 文件按内容锚点定位（不依赖带哈希的文件名）；
# 还原为外科手术式（sidecar 记录三个条目原始字节，按路径定点还原，不整包覆盖）。

MH_PRELOAD_REL = "out/preload/index.cjs"
MH_MAIN_REL = "out/main/index.js"
MH_MARK_PRELOAD = b"modelhubFetchModels"
MH_MARK_MAIN = b"modelhub:fetch-models"
MH_MARK_RENDER = b"__mhPick"


def _load_mh_payload() -> dict:
    src = Path(__file__).resolve().parent / "modelhub_payload.json"
    if not src.is_file():
        raise SystemExit(f"[!] 缺少注入载荷 {src}（应与 zcode_patcher.py 同目录分发）")
    return json.loads(src.read_text(encoding="utf-8"))


def process_modelhub(asar: Path, check_only: bool, revert: bool) -> None:
    side = asar.with_name(asar.name + ".modelhub-patch.json")
    ph = _load_mh_payload()
    raw, header, data_start = _asar_header_raw(asar)
    paths = {p: ent for p, ent in _asar_walk_entries(header)}
    ...
    rend_rel = ph["RENDER_REL"] if ph["RENDER_REL"] in paths else None
    if rend_rel is None:
        anchor = ph["ORIG_ADD_BTN"].encode("utf-8")
        hits = []
        for p, ent in paths.items():
            if not p.startswith("out/renderer/assets/") or not p.endswith(".js"):
                continue
            if ent.get("size", 0) < 100000:
                continue
            if anchor in _asar_entry_bytes(raw, data_start, ent):
                hits.append(p)
        if len(hits) == 1:
            rend_rel = hits[0]
        else:
            print(f"[!] {asar}\n    renderer 内容锚点命中 {len(hits)} 个文件（期望 1），版本结构可能已变，跳过")
            return

    if MH_PRELOAD_REL not in paths or MH_MAIN_REL not in paths:
        print(f"[!] {asar}\n    缺少 preload/main 条目，版本结构可能已变，跳过")
        return

    marks = {MH_PRELOAD_REL: MH_MARK_PRELOAD, MH_MAIN_REL: MH_MARK_MAIN, rend_rel: MH_MARK_RENDER}
    tagged = sum(1 for rel, mark in marks.items()
                 if mark in _asar_entry_bytes(raw, data_start, paths[rel]))
    ...
    if check_only:
        state = "已打" if tagged == 3 else (f"不完整（{tagged}/3）" if tagged else "未打")
        print(f"[*] {asar}\n    模型拉取补丁: {state} | sidecar: {'有' if saved else '无'} | 渲染文件: {rend_rel.split('/')[-1]}")
        return

    if revert:
        if not saved:
            print(f"[.] {asar}\n    没有当前版本的 sidecar 记录（指纹失配或未打过），跳过")
            return
        overwrite = {}
        for rel, mark in marks.items():
            cur = _asar_entry_bytes(raw, data_start, paths[rel])
            if mark not in cur:
                print(f"    [=] {rel.split('/')[-1]} | 已是原版")
                continue
            for f in saved.get("files", []):
                if f.get("path") == rel:
                    overwrite[rel] = base64.b64decode(f["original_b64"])
                    break
        if not overwrite:
            print(f"[.] {asar}\n    三个条目均已是原版，清理 sidecar")
            side.unlink(missing_ok=True)
            return
        _repack_asar(asar, overwrite, set(), tmp_suffix=".modelhub-tmp")
        side.unlink(missing_ok=True)
        _refresh_sidecars_after_repack(asar)
        print(f"[+] {asar}\n    已按 sidecar 定点还原 {len(overwrite)} 个条目（记录已清理）")
        return
    ...
    pre_bytes = _asar_entry_bytes(raw, data_start, paths[MH_PRELOAD_REL])
    main_bytes = _asar_entry_bytes(raw, data_start, paths[MH_MAIN_REL])
    rend_bytes = _asar_entry_bytes(raw, data_start, paths[rend_rel])
    p_txt = pre_bytes.decode("utf-8", "surrogatepass")
    m_txt = main_bytes.decode("utf-8", "surrogatepass")
    r_txt = rend_bytes.decode("utf-8", "surrogatepass")

    if p_txt.count(ph["PRELOAD_ANCHOR"]) != 1:
        print(f"[!] {asar}\n    preload 锚点出现 {p_txt.count(ph['PRELOAD_ANCHOR'])} 次（期望 1），拒绝盲改")
        return
    for key in ("ORIG_ADD_BTN", "ORIG_QPT", "STICKY_OLD", "LE_OLD"):
        cnt = r_txt.count(ph[key])
        if cnt != 1:
            print(f"[!] {asar}\n    renderer 锚点 {key} 出现 {cnt} 次（期望 1），拒绝盲改")
            return

    p_new = p_txt.replace(ph["PRELOAD_ANCHOR"],
                          'exposeInMainWorld("zcode",{' + ph["PRELOAD_INJECT"] + "connectRemote")
    m_new = m_txt + ph["MAIN_HANDLERS"]
    r_new = r_txt.replace(ph["ORIG_ADD_BTN"], ph["ADD_BTN"] + "," + ph["ORIG_ADD_BTN"])
    r_new = r_new.replace(ph["ORIG_QPT"], ph["EDIT_WRAP"])
    r_new = r_new.replace(ph["STICKY_OLD"], ph["STICKY_NEW"])
    r_new = r_new.replace(ph["LE_OLD"], ph["LE_NEW"])
    r_new += ph["HELPER_BLOCK"]

    overwrite = {
        MH_PRELOAD_REL: p_new.encode("utf-8", "surrogatepass"),
        MH_MAIN_REL: m_new.encode("utf-8", "surrogatepass"),
        rend_rel: r_new.encode("utf-8", "surrogatepass"),
    }
    originals = {MH_PRELOAD_REL: pre_bytes, MH_MAIN_REL: main_bytes, rend_rel: rend_bytes}
    new_size = _repack_asar(asar, overwrite, set(), tmp_suffix=".modelhub-tmp")
    side.write_text(json.dumps({
        "asar_size": new_size,
        "renderer_path": rend_rel,
        "files": [{"path": rel, "size": len(b), "original_b64": base64.b64encode(b).decode()}
                  for rel, b in originals.items()],
    }, ensure_ascii=False), encoding="utf-8")
    _refresh_sidecars_after_repack(asar)
    print(f"[+] {asar}\n    模型拉取补丁注入完成（preload/main/renderer 三条目改写）\n"
          f"    记录: {side.name} | 新大小 {new_size:,} 字节")


# ------------------------------------------------- 全消息可编辑（--edit-all，zcode.cjs 两处可逆替换）
# 载荷同样来自 zcode-modelhub-patch 的 engine 补丁（P1 投影放行、P2 解析器去限制）。
# 还原走文本反向替换而非整文件备份，与思考等级补丁（不同函数区域）互不踩踏。

def process_edit_all(target: Path, check_only: bool, revert: bool) -> None:
    side = target.with_name(target.name + ".editall.json")
    ph = _load_mh_payload()
    pairs = [("P1", ph["P1_OLD"], ph["P1_NEW"]), ("P2", ph["P2_OLD"], ph["P2_NEW"])]
    try:
        data = target.read_text(encoding="utf-8", errors="surrogatepass")
    except PermissionError:
        print(f"[!] 无权限读取 {target}")
    ...
    new_cnt = {lbl: data.count(new) for lbl, _, new in pairs}
    old_cnt = {lbl: data.count(old) for lbl, old, _ in pairs}
    applied = sum(1 for lbl, _, _ in pairs if new_cnt[lbl] >= 1 and old_cnt[lbl] == 0)
    state = "已打" if applied == 2 else ("部分（异常）" if applied == 1 else "未打")
    print(f"[*] {target}")
    print(f"    全消息可编辑: {state}")
    ...
    if revert:
        if applied == 0:
            ...
            return
        changed = data
        for lbl, old, new in pairs:
            if new_cnt[lbl] == 1:
                changed = changed.replace(new, old, 1)
        try:
            target.write_text(changed, encoding="utf-8", errors="surrogatepass", newline="\n")
            side.unlink(missing_ok=True)
            print("    [+] 已反向替换还原 P1/P2 两处")
        except PermissionError:
            print("    [!] 无权限写入，未修改")
        return
    ...
    if applied == 2:
        ...
        return
    if applied == 1:
        print("    [!] 状态异常（部分替换），先 --edit-all --revert 再重打")
        return
    for lbl, old, new in pairs:
        if old_cnt[lbl] != 1:
            print(f"    [!] 锚点 {lbl} 出现 {old_cnt[lbl]} 次（期望 1），版本可能不兼容，拒绝盲改")
            return
    changed = data
    for lbl, old, new in pairs:
        changed = changed.replace(old, new, 1)

    tmp = target.with_name(target.name + ".editall-tmp.cjs")   # .cjs 后缀让 node --check 可识别
    try:
        tmp.write_text(changed, encoding="utf-8", errors="surrogatepass", newline="\n")
        node = shutil.which("node")
        if node:
            rc = subprocess.run([node, "--check", str(tmp)], capture_output=True, timeout=120)
            if rc.returncode != 0:
                tmp.unlink(missing_ok=True)
                print(f"    [!] 补丁后语法自检失败（node --check），原文件未动：\n"
                      f"        {rc.stderr.decode(errors='replace')[:300]}")
                return
        os.replace(tmp, target)
        side.write_text(json.dumps({"applied": True}, ensure_ascii=False), encoding="utf-8")
        print("    [+] P1/P2 两处替换已写入" + ("（node --check 通过）" if node else "（未装 node，跳过语法自检）"))
    except PermissionError:
        tmp.unlink(missing_ok=True)
        print("    [!] 无权限写入，未修改")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="ZCode 客户端补丁工具：思维强度透传 + 用量页去截断 + TPS 统计栏 + 模型拉取 + 全消息可编辑（自动探测安装位置）")
    ...
    ap.add_argument("--modelhub", action="store_true",
                    help="注入模型拉取补丁：渠道页「拉取模型」、请求头模拟、视觉探测、删除持久化（asar 重打包级）")
    ap.add_argument("--edit-all", action="store_true",
                    help="全消息可编辑：解除只有最后一条用户消息可编辑的限制（zcode.cjs 两处替换）")
    args = ap.parse_args()

    if args.usage_chart or args.tps_footer or args.modelhub:
        ...
        if args.modelhub:
            mode = "检查" if args.check else ("还原" if args.revert else "打补丁")
            print(f"=== 模型拉取补丁（modelhub），目标 {len(asars)} 处，模式：{mode} ===")
            for a in asars:
                try:
                    process_modelhub(a, args.check, args.revert)
                except PermissionError:
                    print(f"[!] {a}\n    文件被占用（ZCode 正在运行）或无写入权限；完全退出 ZCode 后重试")
    ...
    if args.edit_all:
        mode = "检查" if args.check else ("还原" if args.revert else "打补丁")
        print(f"=== 全消息可编辑补丁，目标 {len(targets)} 处，模式：{mode} ===")
        for t in targets:
            try:
                process_edit_all(t, args.check, args.revert)
            except PermissionError:
                print(f"[!] {t}\n    无写入权限，请检查文件权限后重试")
        if not args.check and not args.revert:
            print("=== 提示：完全退出并重启 ZCode 后生效；升级后需重新执行 ===")
